...
def run_learned_intervention_benchmark(
    ...
    print(f"Overall Mean Relative Error E_rel:  {summary.overall_mean_relative_error:.1%}")
    ...
    print(f"  - True Critical (TP):             {summary.safety_summary.true_critical}")
    print(f"  - False Safe (FN - Hazard):       {summary.safety_summary.false_safe} (Rate: {summary.safety_summary.false_safe_rate:.1%})")
    print(f"  - False Alarm (FP):               {summary.safety_summary.false_alarm}")
    print(f"  - True Safe (TN):                 {summary.safety_summary.true_safe}")
    if summary.failure_time_mae is not None:
        print(f"Failure-Time MAE:                   {summary.failure_time_mae:.1f} steps")
    print("-" * 105)
