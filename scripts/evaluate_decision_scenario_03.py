"""Evaluation Harness for Scenario 3 — Throttle Intervention Is Optimal (Task 5.6).
...
def evaluate_scenario_03(
    model_dir: str | Path = "artifacts/baseline_003",
    benchmark_dir: str | Path = "data/decision_benchmark",
    verbose: bool = True,
) -> Dict[str, Any]:
    """Execute complete decision evaluation for Scenario 3."""
    model_path = Path(model_dir)
    bench_path = Path(benchmark_dir)
    scen_dir = bench_path / "scenarios" / "scenario_03_throttle"
    ...
    if prism_sim is not None:
        ...
        # Channels: 0:T_core, 1:T_cool, 2:P_sys, 3:F_cool, 4:L_cpu, 7:P_elec
        delta_prism_l_cpu = float(np.mean(prism_int_obs[:, 4] - prism_base_obs[:, 4]))
        delta_prism_p_elec = float(np.mean(prism_int_obs[:, 7] - prism_base_obs[:, 7]))
        ...
        delta_prism_l_cpu = delta_prism_p_elec = delta_prism_t_core = delta_prism_f_cool = delta_prism_p_sys = 0.0
    ...
    if selected_oracle_outcome and oracle_base_outcome:
        ...
        delta_orc_p_elec = float(np.mean(orc_int_obs[:, 7] - orc_base_obs[:, 7]))
        ...
        delta_orc_l_cpu = delta_orc_p_elec = delta_orc_t_core = delta_orc_f_cool = delta_orc_p_sys = 0.0

    causal_direction_concordance = (
        (delta_prism_l_cpu < 0 and delta_orc_l_cpu < 0) and
        (delta_prism_p_elec < 0 and delta_orc_p_elec < 0) and
    ...
    result = {
        ...
        "causal_deltas": {
            "PRISM": {
                ...
                "delta_P_elec": delta_prism_p_elec,
    ...
    if verbose:
        print("=========================================================================")
        print("      PRISM DECISION BENCHMARK — SCENARIO 3 EVALUATION REPORT            ")
        print("=========================================================================")
        ...
        print("| :--- | ---: | ---: | :---: | :---: |")
        print(f"| `ΔL_cpu` (CPU Workload)   | {delta_prism_l_cpu:+.2f} % | {delta_orc_l_cpu:+.2f} % | {'✅ PASS' if delta_prism_l_cpu < 0 and delta_orc_l_cpu < 0 else '❌ FAIL'} | `ΔL_cpu < 0` |")
        print(f"| `ΔP_elec` (Power Draw)   | {delta_prism_p_elec:+.2f} kW | {delta_orc_p_elec:+.2f} kW | {'✅ PASS' if delta_prism_p_elec < 0 and delta_orc_p_elec < 0 else '❌ FAIL'} | `ΔP_elec < 0` |")
        ...
        print(f"| `ΔF_cool` (Coolant Flow)   | {delta_prism_f_cool:+.2f} L/min | {delta_orc_f_cool:+.2f} L/min | {'✅ PASS' if abs(delta_prism_f_cool) < 15.0 else '❌ FAIL'} | `Hydrodynamic balance` |")
        print(f"| `ΔP_sys` (System Pressure) | {delta_prism_p_sys:+.2f} bar | {delta_orc_p_sys:+.2f} bar | {'✅ PASS' if abs(delta_prism_p_sys) < 2.0 else '❌ FAIL'} | `Pressure balance` |")
